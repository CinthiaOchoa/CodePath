...
// set inner HTML for the number of games
...
function filterUnfundedOnly() {
    ...
    console.log(unfundedGames.length); // Check how many games are unfunded
...
const displayStr = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames} games. Currently, ${numUnfunded} game${numUnfunded === 1 ? " remains" : "s remain"} 
...
firstGameElement.innerText = `Most Funded Game: ${firstGame.name} - $${firstGame.pledged.toLocaleString()}`;
firstGameContainer.appendChild(firstGameElement);

// do the same for the runner up item
const secondGameElement = document.createElement("p");
secondGameElement.innerText = `Second Most Funded Game: ${secondGame.name} - $${secondGame.pledged.toLocaleString()}`;
...
document.getElementById('search-btn').addEventListener('click', function () {
    ...
    // Scroll smoothly to the game container
    if (filteredGames.length > 0) {
        gamesContainer.scrollIntoView({ behavior: 'smooth' });
    } else {
        alert('No games found. Try another search term!');
    }
});
