...
function addGamesToPage(games) {
    ...
    for (let i=0; i < games.length ; i++) {
        ...
        gamesCard.innerHTML = `<img class="game-img" src="${games[i].img}" /> <div class="name">${games[i].name}</div> <div>${games[i].description}</div>  <div>Pledged: ${games[i].pledged}</div>`;
        // about each game
...
document.getElementById("num-contributions").innerHTML = totalContributions
...
let count = 0;
const unfundedGames = GAMES_JSON.filter((game) => {
    game.pledged < game.goal ? count++ : 0;
});
// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of ${totalRaised.toLocaleString()} has been raised for ${GAMES_JSON.length - count} games. Currently, ${count} game remains unfunded. We need your help to fund these amazing games`
