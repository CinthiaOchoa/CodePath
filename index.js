...
const displayStr =
  numOfUnfundedGames > 1
    ? `A total of $${totalRaised.toLocaleString("en-US")} for ${
        GAMES_JSON.length
      } games. Currently, ${numOfUnfundedGames} games remians unfunded. We need your help to fund these amazing games!`
    : `A total of $${amtOfFunded.toLocaleString("en-US")} for ${
        GAMES_JSON.length
      } games. Currently, 1 games remians unfunded. We need your help to fund these amazing games!`;
// create a new DOM element containing the template string and append it to the description container
const displayPara = document.createElement("div");
...
//search the game by their names
function searchGame() {
  deleteChildElements(gamesContainer);
  ...
  const searchGameName = GAMES_JSON.filter((game) => {
    return game.name.toLowerCase() == input;
  });
  addGamesToPage(searchGameName);
}
